...
1. Use `neovim_vim_grep` to search for "$ARGUMENTS" across the project.
2. Identify the most relevant files (max 4) based on the results.
3. Open each file with `neovim_vim_file_open`.
4. Arrange them using `neovim_vim_window split` or `vsplit` so the user can see them side by side.
